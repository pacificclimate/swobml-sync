...
- [x] Repository is a git repository with a Python package and console entry point named `swobml-sync`
- [x] `partner` and `dir` are required; `--days-back` (default 2), `--date` (repeatable), `--retention-days` (default 65), `--workers` (default 8), `--manifest`, `--log-level` (default INFO) are all present
- [x] Every flag has a `SWOBML_*` environment-variable fallback; a command-line argument overrides the env var when both are set
- [x] Invalid input (e.g. a malformed `--date`, a non-integer `--workers`) exits non-zero with a readable message
- [x] `--help` documents all flags; a test asserts argument parsing and env-var precedence
